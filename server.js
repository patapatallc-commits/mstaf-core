...
app.post("/webhook", async (req, res) => {
  try {
    ...
    // MAIN MENU
    // =========================
    if (!session.stage || session.stage === "MENU") {
      session.stage = "MENU";
...
${serviceMenu()}`
    ...
    if (session.stage === "PRINT_FILE_UPLOADED_ACTION") {
      if (lower === "1") {
        ...
        await sendMessage(
          from,
          `Your request has been sent to our Agent team.

Please send your instructions (text or voice).`
        );
        return res.sendStatus(200);
      }
...
Send your payment proof after payment.`
        );
        session.stage = "MENU";
        return res.sendStatus(200);
      }
...
Our team will contact you shortly.`
      ...
      await sendMessage(from, "Send instruction as text or voice.");
      return res.sendStatus(200);
    }
