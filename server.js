...
const audioBoostTranslations={
  en:{button:'🔊 Boost & Replay',label:'Preview volume',ready:'Tap the purple Play Recording button or Boost & Replay. Turn up your phone media volume too.',playing:'Preview is playing with boosted volume. Confirm that you can hear your voice clearly.',unsupported:'This browser cannot amplify the preview. Use the phone volume buttons and record closer to the microphone.'},
...
let previewAudioContext=null,previewMediaSource=null,previewGainNode=null,previewCompressor=null;
...
function showAudioPreview(fileOrBlob,label){clearAudioPreviewUrl();audioPreviewConfirmed=false;recordedAudioUrl=URL.createObjectURL(fileOrBlob);recordedAudioPreview.pause();recordedAudioPreview.src=recordedAudioUrl;recordedAudioPreview.muted=false;recordedAudioPreview.volume=1;recordedAudioPreview.load();audioBoostRange.value='3';syncPreviewBoost();recordedAudioPreview.classList.remove('hidden');audioBoostRow.classList.remove('hidden');playRecordBtn.classList.remove('hidden');recordAgainBtn.classList.remove('hidden');audioPreviewStatus.textContent='🔊 '+label+' '+audioBoostUi.ready;}
...
async function playBoostedPreview(){if(!recordedAudioPreview.src)return;try{const amplified=await ensurePreviewAmplifier();recordedAudioPreview.muted=false;recordedAudioPreview.volume=1;recordedAudioPreview.currentTime=0;await recordedAudioPreview.play();audioPreviewConfirmed=true;audioPreviewStatus.textContent='✅ '+(amplified?audioBoostUi.playing:audioBoostUi.unsupported);}catch(error){audioPreviewConfirmed=false;audioPreviewStatus.textContent='❌ This recording could not play. Record again or upload an existing audio file.';}}
  ...
  try {
    ...
    if (introAudioIndex >= 0) {
      audioFilters.push(
        ...
        `aresample=48000,aformat=sample_fmts=fltp:sample_rates=48000:channel_layouts=stereo,` +
        `loudnorm=I=-14:TP=-1.0:LRA=8,volume=1.18,` +
